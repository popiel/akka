  ...
  override val supervisorStrategy =
    OneForOneStrategy() {
      case NonFatal(e) ⇒
        ...
        self ! InternalClusterAction.Shutdown
        Stop
    }
  def receive = {
    case InternalClusterAction.GetClusterCoreRef ⇒ sender ! coreDaemon
    case InternalClusterAction.Shutdown          ⇒ Cluster(context.system).shutdown()
  }
}
